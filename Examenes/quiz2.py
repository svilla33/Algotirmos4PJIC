...
def validar_columna(tablero, num_columna):
    ...
    for i in range(9):
        fila = tablero[i]
        vColumna = fila[num_columna]
        
        columna.add(vColumna) 
    


    if(columna == NUMEROS_VALIDOS):
        return True
    else:
        return False
    pass
...
def validar_subcuadro(tablero, fila_inicio, col_inicio):
    ...
    for i in range(3):
        for j in range(3):  
            fila = tablero[i+fila_inicio]
            subFila = fila[j+col_inicio]            
            subcuadro.add(subFila) 
            
    if(subcuadro == NUMEROS_VALIDOS):
        return True
    else:
        return False
